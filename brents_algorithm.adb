--  Brents_Algorithm body — powers-of-two / teleport search, μ recovery,
--  naive reference, graph builders.
...
is
   ...
   --  Phase 1: powers-of-two search. Returns Meeting_Point and Lambda
   --  via out params; Meeting = Null_Index means no cycle.
   ...
   is
      ...
      Lam            : Natural := 1;
      Guard          : Natural := 0;
      Limit          : constant Natural := 4 * Next'Length + 8;
   begin
      Tortoise := Node_Index (Start);
      Hare     := Fwd (Next, Tortoise);
   ...
   begin
      ...
      --  one step at a time until they meet at x_μ.
      Tortoise := Node_Index (Start);
      Hare     := Node_Index (Start);
